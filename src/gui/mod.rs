use iced::{
  button, executor, keyboard, pane_grid, pane_grid::Configuration, scrollable, Align, Application,
  Button, Column, Command, Container, Element, HorizontalAlignment, Length, PaneGrid, Row,
...
impl Application for GUI {
  ...
  fn new(_flags: ()) -> (Self, Command<Message>) {
    let configuration = Configuration::Split {
      ...
      ratio: 0.5,
      a: Box::new(Configuration::Pane(Content::new("Editor".to_string()))),
      b: Box::new(Configuration::Split {
        axis: pane_grid::Axis::Horizontal,
        ratio: 0.7,
        a: Box::new(Configuration::Pane(Content::new(
          "Bitmap Display".to_string(),
        ))),
        b: Box::new(Configuration::Pane(Content::new("Console".to_string()))),
      }),
    };

    let panes = pane_grid::State::with_configuration(configuration);

    (GUI { panes, focus: None }, Command::none())
  }
  ...
  fn update(&mut self, message: Message) -> Command<Message> {
    match message {
      ...
      // }
      _ => {
        println!("Message {:?} ignored", message)
      }
    }
  ...
  fn view(&mut self) -> Element<Message> {
    ...
    let pane_grid = PaneGrid::new(&mut self.panes, |pane, content| {
      ...
      let title = Row::with_children(vec![Text::new(&content.title).into()]).spacing(5);
